...
> These are deliberately **multi-service**. A good answer names the services, draws the data/permission flow, states the trade-off, and calls out failure modes — it does not just define one service. Each card has a **Setup**, a **Question**, a **Strong answer** outline, the **Services in play**, and the **follow-ups** an interviewer pushes. Reason out loud; an interviewer scores the path, not just the destination.
